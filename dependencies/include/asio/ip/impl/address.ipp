...
address::address(
    const clmdep_asio::ip::address_v4& ipv4_address) ASIO_NOEXCEPT
  : type_(ipv4),
...
address::address(
    const clmdep_asio::ip::address_v6& ipv6_address) ASIO_NOEXCEPT
  : type_(ipv6),
    ipv4_address_(),
...
address make_address(const char* str,
    clmdep_asio::error_code& ec) ASIO_NOEXCEPT
{
  clmdep_asio::ip::address_v6 ipv6_address =
    clmdep_asio::ip::make_address_v6(str, ec);
  if (!ec)
    return address(ipv6_address);

  clmdep_asio::ip::address_v4 ipv4_address =
    clmdep_asio::ip::make_address_v4(str, ec);
  if (!ec)
