...
public:
   ...
   */
  results_type resolve(const query& q, clmdep_asio::error_code& ec)
  {
   ...
   */
  results_type resolve(ASIO_STRING_VIEW_PARAM host,
      ASIO_STRING_VIEW_PARAM service, clmdep_asio::error_code& ec)
  {
  ...
  results_type resolve(const protocol_type& protocol,
      ASIO_STRING_VIEW_PARAM host, ASIO_STRING_VIEW_PARAM service,
      resolver_base::flags resolve_flags, clmdep_asio::error_code& ec)
  {
    basic_resolver_query<protocol_type> q(
        protocol, static_cast<std::string>(host),
        static_cast<std::string>(service), resolve_flags);
  ...
  ASIO_INITFN_AUTO_RESULT_TYPE(ResolveHandler,
      void (clmdep_asio::error_code, results_type))
  async_resolve(const query& q,
      ASIO_MOVE_ARG(ResolveHandler) handler
        ASIO_DEFAULT_COMPLETION_TOKEN(executor_type))
  {
    return clmdep_asio::async_initiate<ResolveHandler,
      void (clmdep_asio::error_code, results_type)>(
   ...
   * function is guaranteed to return a non-empty range.
   */
  results_type resolve(const endpoint_type& e, clmdep_asio::error_code& ec)
  {
...
private:
  ...
  public:
        ...
        const Query& q) const
    {
      ...
      clmdep_asio::detail::non_const_lvalue<ResolveHandler> handler2(handler);
...
# if defined(ASIO_WINDOWS_RUNTIME)
  clmdep_asio::detail::io_object_impl<
    clmdep_asio::detail::winrt_resolver_service<InternetProtocol>,
    Executor> impl_;
# else
  clmdep_asio::detail::io_object_impl<
    clmdep_asio::detail::resolver_service<InternetProtocol>,
    Executor> impl_;
# endif
};
