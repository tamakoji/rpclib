 ...
 * tcp::socket s(my_context);
 * clmdep_asio::connect(s, e.begin(), e.end()); @endcode
 */
template <typename Protocol, typename Executor, typename Iterator>
Iterator connect(basic_socket<Protocol, Executor>& s,
 ...
 * tcp::socket s(my_context);
 * clmdep_asio::error_code ec;
 * clmdep_asio::connect(s, e.begin(), e.end(), ec);
 * if (ec)
 * {
 *   // An error occurred.
 * } @endcode
 ...
 * tcp::resolver::query q("host", "service");
 * tcp::socket s(my_context);
 * tcp::endpoint e = clmdep_asio::connect(s,
 ...
 * tcp::resolver::results_type e = r.resolve(q);
 * tcp::socket s(my_context);
 * tcp::resolver::results_type::iterator i = clmdep_asio::connect(
 ...
 *   if (!ec)
 *   {
 *     clmdep_asio::async_connect(s, results, connect_handler);
 *   }
 * }
 *
 * // ...
 ...
 *   if (!ec)
 *   {
 *     clmdep_asio::async_connect(s, results,
