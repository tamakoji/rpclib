...
win_iocp_io_context::win_iocp_io_context(
    clmdep_asio::execution_context& ctx, int concurrency_hint, bool own_thread)
    ...
    concurrency_hint_(concurrency_hint)
{
  ...
  if (own_thread)
  {
    ...
    thread_.reset(new clmdep_asio::detail::thread(thread_function(this)));
  }
}
...
clmdep_asio::error_code win_iocp_io_context::register_handle(
...
size_t win_iocp_io_context::run(clmdep_asio::error_code& ec)
{
...
size_t win_iocp_io_context::run_one(clmdep_asio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::wait_one(long usec, clmdep_asio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll(clmdep_asio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll_one(clmdep_asio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::do_one(DWORD msec, clmdep_asio::error_code& ec)
{
  for (;;)
  {
