...
public:
   ...
   */
  explicit strand(clmdep_asio::io_context& io_context)
    : service_(clmdep_asio::use_service<
        clmdep_asio::detail::strand_service>(io_context))
  {
  ...
  clmdep_asio::io_context& context() const ASIO_NOEXCEPT
  {
  ...
  /// (Deprecated: Use clmdep_asio::dispatch().) Request the strand to invoke
  /// the given handler.
  /**
...
#if !defined(ASIO_NO_DEPRECATED)
  /// (Deprecated: Use clmdep_asio::post().) Request the strand to invoke the
...
private:
  ...
  mutable clmdep_asio::detail::strand_service::implementation_type impl_;
};
