...
template <>
class promise_handler_selector<void(clmdep_asio::error_code)>
...
template <typename Arg>
class promise_handler_selector<void(clmdep_asio::error_code, Arg)>
...
#define ASIO_PRIVATE_PROMISE_SELECTOR_DEF(n) \
  ...
  template <typename Arg, ASIO_VARIADIC_TPARAMS(n)> \
  class promise_handler_selector< \
    void(clmdep_asio::error_code, Arg, ASIO_VARIADIC_TARGS(n))> \
