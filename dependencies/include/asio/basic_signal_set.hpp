 ...
 * clmdep_asio::signal_set signals(my_context, SIGINT, SIGTERM);
 *
...
public:
   ...
   * @code clmdep_asio::signal_set signals(ex);
   ...
   *
   * @note This constructor is equivalent to performing:
   * @code clmdep_asio::signal_set signals(context);
   * signals.add(signal_number_1); @endcode
   */
  template <typename ExecutionContext>
   ...
   * @note Removes all queued notifications.
   */
  ASIO_SYNC_OP_VOID clear(clmdep_asio::error_code& ec)
  {
    impl_.get_service().clear(impl_.get_implementation(), ec);
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
   ...
   */
  template <
    ASIO_COMPLETION_TOKEN_FOR(void (clmdep_asio::error_code, int))
  ...
  {
    return async_initiate<SignalHandler, void (clmdep_asio::error_code, int)>(
