...
scheduler::scheduler(clmdep_asio::execution_context& ctx,
    int concurrency_hint, bool own_thread)
  : clmdep_asio::detail::execution_context_service_base<scheduler>(ctx),
    ...
    thread_(0)
{
  ...
  if (own_thread)
  {
    ...
    thread_ = new clmdep_asio::detail::thread(thread_function(this));
  }
}
...
std::size_t scheduler::run(clmdep_asio::error_code& ec)
{
...
std::size_t scheduler::run_one(clmdep_asio::error_code& ec)
{
  ec = clmdep_asio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::wait_one(long usec, clmdep_asio::error_code& ec)
{
  ec = clmdep_asio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll(clmdep_asio::error_code& ec)
{
  ec = clmdep_asio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll_one(clmdep_asio::error_code& ec)
{
  ec = clmdep_asio::error_code();
  if (outstanding_work_ == 0)
  {
