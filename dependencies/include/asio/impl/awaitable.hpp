...
public:
  ...
  {
    return clmdep_asio::detail::thread_info_base::allocate(
        clmdep_asio::detail::thread_info_base::awaitable_frame_tag(),
        clmdep_asio::detail::thread_context::thread_call_stack::top(),
        size);
  }

  void operator delete(void* pointer, std::size_t size)
  {
    clmdep_asio::detail::thread_info_base::deallocate(
        clmdep_asio::detail::thread_info_base::awaitable_frame_tag(),
        clmdep_asio::detail::thread_context::thread_call_stack::top(),
        pointer, size);
  }
  ...
  void set_error(const clmdep_asio::error_code& ec)
  {
    this->set_except(std::make_exception_ptr(clmdep_asio::system_error(ec)));
  }
...
struct coroutine_traits<clmdep_asio::awaitable<T, Executor>, Args...>
{
  typedef clmdep_asio::detail::awaitable_frame<T, Executor> promise_type;
};
