...
# endif // !defined(ASIO_NO_DEPRECATED)
      ...
      expiry_time_(other.expiry_time_)
  {
    ...
    other.ec_ = clmdep_asio::error_code();
...
protected:
  int_type underflow()
  {
    ...
    ec_ = clmdep_asio::error::operation_not_supported;
    ...
    for (;;)
    {
      ...
      {
        ec_ = clmdep_asio::error::timed_out;
        return traits_type::eof();
      }
      ...
      detail::buffer_sequence_adapter<mutable_buffer, mutable_buffer>
        bufs(clmdep_asio::buffer(get_buffer_) + putback_max);
      ...
      if (bytes == 0)
      {
        ec_ = clmdep_asio::error::eof;
        return traits_type::eof();
      }

      // Operation failed.
      if (ec_ != clmdep_asio::error::would_block
          && ec_ != clmdep_asio::error::try_again)
        return traits_type::eof();
  ...
  int_type overflow(int_type c)
  {
    ...
    if (put_buffer_.empty())
    {
      ...
      output_buffer = clmdep_asio::buffer(&ch, sizeof(char_type));
    }
    else
    {
      output_buffer = clmdep_asio::buffer(pbase(),
...
private:
  ...
  void connect_to_endpoints(EndpointIterator begin, EndpointIterator end)
  {
    ...
    ec_ = clmdep_asio::error::not_found;
    for (EndpointIterator i = begin; i != end; ++i)
    {
      ...
      // Operation failed.
      if (ec_ != clmdep_asio::error::in_progress
      ...
      ec_ = clmdep_asio::error_code(connect_error,
