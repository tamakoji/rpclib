...
public:
  ...
  prepared_buffers_type prepare(std::size_t max_size)
  {
    ...
    Buffer_Iterator next = clmdep_asio::buffer_sequence_begin(buffers_);
    Buffer_Iterator end = clmdep_asio::buffer_sequence_end(buffers_);
    ...
    while (next != end && max_size > 0 && (result.count) < result.max_buffers)
    {
      ...
      result.elems[result.count] = clmdep_asio::buffer(next_buf, max_size);
...
public:
  ...
  {
    return clmdep_asio::buffer(buffer_ + total_consumed_, max_size);
  }
...
public:
  ...
  boost::array<Buffer, 2> prepare(std::size_t max_size)
  {
    ...
    result[0] = clmdep_asio::buffer(result[0] + total_consumed_, max_size);
    result[1] = clmdep_asio::buffer(
